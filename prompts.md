...
2. **Company & funding context**: about the competitor only, one short paragraph (2-3 sentences max). Include ownership/funding facts that could change what a rep says or does (recent funding, layoffs, acquisition, leadership change, parent-company ownership). If the competitor is owned by a larger group, you can note the group's general reputation (e.g. known for an acquisitive "roll-up" model) as background color, but phrase it as industry reputation, not as a confirmed fact about this specific competitor. Otherwise write exactly "Nothing notable."
Before finalizing this section, check whether an equivalent situation (acquisition, parent group, similar funding structure) applies to my own company too — if it does, tell me directly instead of silently leaving my company looking cleaner than the competitor by omission. This section is background knowledge for the rep, not something to phrase as a line they'd say out loud — that belongs in Objection handling instead, if and only if a prospect would plausibly bring it up unprompted.
...
7. **Objection handling**: 3-5 real phrases a prospect would say, each with a response a rep can say out loud without sounding defensive. Keep every response to 1-2 short sentences max — this is spoken language, not a report; cut anything a rep wouldn't actually say mid-conversation. If the material I gave you already answers the objection, give that answer directly and confidently — don't default to "let me check" or "good question, I'll get back to you" when you actually know the answer; reserve that kind of deferral for cases where the information is genuinely missing. Don't invent an objection whose honest answer would require information we don't have (e.g. don't write a "their price is cheaper" objection if we have no data on their pricing) — skip it instead. (This section repeats as a block per objection, so 5 is fine if you have 5 good ones.)
...
- Proof points: 4-6 short, punchy stats or facts (not full sentences you'd read in a report — phrase each as a standalone one-liner under ~20 words), each with a source AND a date the fact was checked. Pick your strongest 4-6, don't dump every fact you have. This section exists to help us win the deal — only include facts that support us or create legitimate, source-backed doubt about the competitor. Never include a positive competitor customer quote or testimonial here, even if it's publicly available — that undermines the point of the section. If the strongest material you found about the competitor happens to be flattering to them, leave it out rather than including it for "balance."
...
Tone: direct, factual, respectful of the competitor. No marketing fluff, no unfair exaggeration of the competitor's weaknesses. You can use verified facts to create legitimate doubt about the competitor (pricing opacity, ownership structure, missing capabilities) — that's fair game — but never state something as fact that isn't confirmed, and if an argument relies on general reputation or pattern (e.g. "companies like this parent group tend to...") rather than something confirmed about this specific competitor, phrase it that way so the rep knows the limits of the argument if a prospect pushes back. A rep should be able to say any sentence in this out loud on a call without sounding either defensive or dishonest.
